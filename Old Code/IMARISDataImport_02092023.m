function IMARISDataImport_02092023(celllines_color, celllines, maindir, root, type, tp, pool)
            ...
            for i = 1:length(files) %for every excel file
                ...
                for d = 1:length(comp)
                    if ~contains(full_sheets, comp(d))
                        error = append('Parameter Sheet ', comp(d) , ' undetected in file ', cellnum_filename, '. Check the code and/or file for errors.')
                    end
                end
                ...
                for k = 1:length(sheets_index) %for every relevant sheet in the excel file
                    ...
                    elseif contains(active_sheet, "Track")
                        continue
