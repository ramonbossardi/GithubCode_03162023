function IMARISDataImport_01152023_LiveNontrack(celllines_color, celllines, maindir, root, tp, pool)
    ...
    progressbar([], [], .4, 0, 0)
    [X, dpg_data] = parameter_calc(X, T_labels, objectID);
    ...
    function [X_new, mmdist_data] = parameter_calc(X, T_labels, objectID)
        ...
        [peaks, peaks_idx] = findpeaks(flip(T_labels));
        cellnum = length(peaks)+1;
        mmdist_data = cell(cellnum,1);
        start_index = [1; flip((length(T_labels) - peaks_idx + 2))];
        ...
        for i = 1:length(mmdist_data)  %for every cell's Mito Mito Distance array
            ...
            if i == length(mmdist_data)
                current_T_labels = T_labels(start_index(i):end);
            else
                current_T_labels = T_labels(start_index(i): start_index(i+1) - 1);
            end
            ...
            for j = 1:length(current_mmdist_data)  %for every object in that array
                ...
                %Debugging
                %{
                if isempty(obj_mmdist)
                    error1_tp = [error1_tp; current_T_labels(j)];
                end
                if isempty(min(obj_mmdist))
