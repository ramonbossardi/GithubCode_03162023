function IMARISDataImport_01092023_AllTimepoints(celllines_color, celllines, maindir, root)
    ...
    progressbar([], [], 0, 0, 0)
    [X, X_labels, T_labels, objectID, cell_list] = parameter_import(import_dir);
    

    %~~PART 2: SECONDARY PARAMETER CALCULATION~~
    progressbar('MainScript', root, 'Importing', 'Parameter Calculation', '')
    progressbar([], [], .4, 0, 0)
    [X, X_labels, T_labels, objectID, dpg_data] = parameter_calc(X, X_labels, T_labels, objectID);
    ...
    function [X, X_labels, T_labels, objectID, cell_list] = parameter_import(location)
            %Loading Bar
            progressbar([], [], [], 0, 0) 
            dir(location)
            ...
            for i = 1:length(files) %for every excel file
                ...
                full_sheets = string(sheetnames(ds1, modfile_i(i))); %The list of sheet names for the file
                ...
                for k = 1:length(sheets_index) %for every relevant sheet in the excel file
                    ...
                    elseif contains(active_sheet, 'Shortest Distance')
                        ...
                        surfdist_string = [strcat("cell", string(cellnum), " DAPI"), strcat("dapi cell", string(cellnum)), ...
                            ...
                            strcat("cell ", string(cellnum), " wt nuc")];
                ...
                for c = 1:length(celllines)
                    if contains(modifiedfiles(i), celllines(c))
            ...
            for i = 1:length(T_labels)
                currentTP = T_labels(i);
                if TP ~= currentTP 
                    if k <= 3 && k ~= 0
                        removeTP = [removeTP (i-k):(i-1)];
                    end
                    k = 0;
                    TP = currentTP;
                end
    ...
    function [X_new, X_labels_new, T_labels_new, objectID_new, mmdist_data] = parameter_calc(X, X_labels, T_labels, object_ID)
        X_labels_new = X_labels;
        T_labels_new = T_labels;
        objectID_new = object_ID;
        ...
        for i = 1:length(mmdist_data)  %for every cell's Mito Mito Distance array
            ...
            current_T_labels = T_labels(current_ID:(current_ID+length(current_mmdist_data)-1));
            for j = 1:length(current_mmdist_data)  %for every object in that array
               
                T_range = find(current_T_labels == current_T_labels(j));

                obj_mmdist = nonzeros(current_mmdist_data(j,T_range)); %removes the zero representing the same object

                if length(obj_mmdist) <= 3
                    error_tp = [error_tp; current_T_labels(j)];
                    removing_idx = [removing_idx; (current_ID + j - 1)];
                    continue
                end
            ...
            current_ID = current_ID + length(current_mmdist_data);
        end

        %Put these new parameters in the dataset
        X_new(removing_idx, :) = [];
        X_labels_new(removing_idx) = [];
        T_labels_new(removing_idx) = [];
        objectID_new(removing_idx) = [];
        volume(removing_idx) = [];
        pos_z(removing_idx) = [];
