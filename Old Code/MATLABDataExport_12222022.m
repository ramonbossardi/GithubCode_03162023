function MATLABDataExport_12222022(dir, root)
...
progressbar([], [], .5, 0, 0)
cellline_dpgexport(M.X, M.X_labels, M.dpg_data, M.celllines, [root ' Full/DPGCalcData']);
cd(workspace_dir)

progressbar('MainScript', root, 'Exporting', 'Alt Data Exportation', '')
progressbar([], [], .8, 0, 0)
cellline_altdataexport(M.X, M.X_labels, M.objectID, M.param_labels, M.dpg_data, M.celllines, M.cell_list, [root ' Full/Alt']);
end
...
function cellline_altdataexport(X, X_labels, objectID, param_labels, dpg_data, celllines, cell_list, filename)
    ...
    writetable(A3, 'Volume_SumData.csv')
    ...
    A4.Properties.VariableNames = cellstr([' ' sort(celllines)]);
    writetable(A4, 'Area_SumData.csv')
    cd ../
    ...
    progressbar([], [], [], [], 7/8)
    numobj_data = zeros(maxcellnum, length(celllines));
