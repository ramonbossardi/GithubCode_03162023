function MATLABDataExport_02092023(dir, root, type)
...
function cellline_altdataexport(X, X_labels, T_labels, objectID, param_labels, dpg_data, celllines, cell_list, filename)
        ...
        for j = 1:length(celllines)
            ...
            for k = 1:maxcellnum
                if any(cl_cell_num == k)
                            temp_meandata = [temp_meandata; X_mean((start+x),i)];
                    temp_sumdata = [temp_sumdata; X_sum((start+x), i)];
                    x = x + 1;
                else
    ...
    for i = 1:length(celllines)
        ...
        for k = 1:maxcellnum
                if any(cl_cell_num == k)
                    temp_normdata = [temp_normdata; norm_data((start+x),:)];
                    x = x + 1;
                else
    ...
    for i = 1:length(celllines)
        ...
        for k = 1:maxcellnum
                if any(cl_cell_num == k)
                    temp_numdata = [temp_numdata; length(cell2mat(dpg_data(x)))];
                    x = x + 1;
                else
